...
- Added one Git-independent validation runner for checkouts and extracted ZIPs.
  CI now calls the same archive-safe command plan after provisioning its pinned
  dependencies, while whitespace hygiene remains explicitly checkout-only.
...
- Protected final-frame extraction outputs from accidental or partial replacement. Existing destinations are refused by default, `--force` is required for an intentional regular-file replacement, and FFmpeg now streams decoded frames back to the helper instead of reopening a mutable staging pathname. A bounded independent FFmpeg decode probe rejects a structurally delimited but undecodable retained PNG before publication. The complete retained frame is written through an owned handle before atomic publication; every POSIX staging, exchange-probe, and publication-anchor component is bounded by that directory's `PC_NAME_MAX` (including 14-byte filesystems) without copying a near-limit output stem or suffix. Windows retains the exact non-reparse destination directory before decode, rejects parent/junction redirection and every reserved device stem (including `COM¹`–`COM³` and `LPT¹`–`LPT³`) again at the native boundary, supports extended-length valid names, and renders console-safe Unicode status/errors without changing a completed transaction. POSIX publication links the verified open inode rather than a swappable staging name, and exact descriptor-checked cleanup covers failures immediately after private-directory, stage-file, or either exchange-probe creation—even when the first probe `fstat` fails. Linux replacements preserve descriptor-readable user attributes and access ACLs alongside owner/group/mode only when the process proves effective `CAP_SYS_ADMIN`, a controlled destination directory, privileged-namespace visibility, and filesystem support for `renameat2(RENAME_EXCHANGE)` before decode; the privileged workflow now explicitly reports and skips success-path regressions when hosted-runner policy withholds that capability while proving the helper fails closed. The old target descriptor remains open through decode; publication exchanges and verifies both names, rolling back if the target was swapped after the final precheck. Windows replacements initially refuse when owner/group/DACL/mandatory-label policy differs from the protected stage, a named stream exists, or a policy-bearing attribute exists. Commit then uses `ReplaceFileW` with no ignore flags and a unique backup of the actual boundary-time target; target and stage identity, bytes, canonical SDDL policy, streams, and attributes are post-verified before deletion, with an exact handle-bound rollback on any late winner or policy mutation. Ordinary unprivileged Linux, security/trusted/unsupported system attributes, unsupported enumeration or exchange, non-Linux POSIX, or otherwise unauditable policy metadata makes replacement fail closed. New files remain available, honor the caller's umask, and need no capability. WebP uses FFmpeg's `libwebp` encoder, late destination collisions are preserved, failed extraction leaves the prior output untouched, and Windows cleanup deletes the owned handle rather than a re-looked-up path.

## [6.7.0] — 2026-08-01

### Added

- Recorded that **a newer model line exists**. ByteDance's [official model page](https://seed.bytedance.com/en/seedance2_5) confirms Seedance 2.5, and [Dreamina's official product page](https://dreamina.capcut.com/seedance/seedance-2-5) says it is live on Dreamina. Neither primary page gives an exact launch date, and API or other-surface availability remains unconfirmed in this review. `api-status` now opens with that source boundary, `model-name-map` gains a 2.5 boundary entry with a standing rule never to normalize 2.5 to 2.0, the root skill's source gate requires establishing the **model line** and not just the surface, and the README says it above the platform paragraph. Technology-press and provider access reports remain below the `confirmed` bar. The rule that falls out of it: craft transfers across model lines; platform numbers never do.
