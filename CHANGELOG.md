...
- Added `scripts/prompt_architecture_stress.py` and `evals/prompt-architecture-stress.json`: 102 prompts covering 34 briefs across every mode, written three ways — untrained/listicle style, the shape the beginner example demonstrated, and the `seedance-prompt` Director Formula — and scored mechanically on opening authority, length fit, slop density, craft coverage, structure, and reference integrity. The Director Formula arm scores 3.88 against the `eval-rubric` bar; the other two fail it. Wired into CI with `--strict`, so a change that degrades the doctrine arm fails the build instead of reaching users as "the prompts feel average".
...
- Rewrote the beginner example in all six quickstarts. The "Directed (strong)" prompt was 29 words — under the 40-110 band the root skill states — and opened on `Medium close-up, eye-level`, spending the highest-weight position on framing metadata, which is the inversion `seedance-prompt` warns against when it says to put the subject and primary action first. Replaced with a 71-word subject-first prompt and the explanation of why the order is the lesson.
