...
- Added `assets/masthead-outlines.json` and `scripts/build_masthead_outlines.py`: the masthead's display type as vector outlines, shaped with HarfBuzz kerning from Bodoni Moda (SIL OFL, attribution and instance axes recorded in the file), with optical size tracking the rendered size rather than a fixed instance.
