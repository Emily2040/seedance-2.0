...
2. Load `[ref:directors-read]` before writing the brief. Narrative, story, and performance ideas complete its ten-field read before prompt compilation; utility, product-only, and abstract ideas record its two-line non-narrative refusal instead. Then apply one visible beat, one motivated camera move, one motivated light source, sound intent, and the directing coherence rule. Load `[ref:directing-engine]`, `[ref:capability-map]`, `[ref:allocation-model]`, and the source or professional gates only when the shot needs their deeper machinery.
...
10. Direction: run the mandatory `[ref:directors-read]` gate before drafting. For a narrative lane, derive one intention from the completed read and make camera, lens, light, blocking, performance, and sound serve it instead of picking a "cinematic look". For a non-narrative lane, serve the utility intent without inventing character drama. Load `[ref:directing-engine]` when scenes need distinct treatment, one directorial voice must hold across many clips, or the right setup is genuinely unclear.
...
| Situation | Load |
|---|---|
| Any brief before prompt drafting or compilation; mandatory narrative read or non-narrative refusal | `[ref:directors-read]` |
