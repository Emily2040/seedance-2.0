...
5. Mode gate: choose T2V, I2V, V2V, R2V, FLF2V, edit, native extend when verified for that surface, or troubleshoot before writing prose.
   ...
   Availability is per operation, not per provider. Never infer an endpoint, entitlement, request field, duration ceiling, or fallback from a provider's name or from a sibling operation on the same surface. When the exact surface operation is unknown, disabled, or undocumented, say so and stop rather than guessing a plausible one.
