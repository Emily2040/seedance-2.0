...
Speed does not bypass story judgment. Load `[ref:directors-read]` before producing the compact brief. Complete its ten-field internal record for every narrative, story, or performance idea; for a utility, product-only, abstract, VFX, or ambient idea with no requested agency or performance, record its two-line non-narrative intent and refusal instead. Never reconstruct the read from remembered craft.
...
Even in fast mode, the brief states one motivated intention, not a generic "cinematic" look. For narrative work, derive it from the completed `[ref:directors-read]` record and translate the turn, visible suppressed behavior, and non-transferable detail into filmable or audible carriers. For non-narrative work, serve the utility intent without inventing want, power, conflict, or subtext. Load `[ref:directing-engine]` only when the right setup for the scene is genuinely unclear.

## Compact Brief Pattern

Internal lane record from `[ref:directors-read]`, then: `Mode: [T2V/I2V/V2V/R2V]. Subject: [anchor]. Beat: [before -> action -> final state]. Camera: [one move]. Light/style: [physical source and safe descriptor]. Sound: [dialogue/ambience/SFX/music/silence]. Constraints: [identity, IP, safety, product, prompt budget].`
