...
2. Classify every clip with the [Director's Read](../../references/directors-read.md) and record its explicit `directors_read_lane` as `narrative` or `non_narrative`; never infer psychology from `story.medium`. This per-clip lane lets utility inserts coexist with performed beats in one sequence. Before a clip classified in the narrative lane is compiled, complete its ten-field internal record, including one non-transferable detail and one stock-solution refusal. For observation/performance without a dramatic story turn or for utility, use the non_narrative lane and record the two-line intent and refusal; do not invent drama or suppress the requested expression. Then use [directing-engine](../../references/directing-engine.md) to set one directorial voice for the whole project when the sequence needs it and plan the long-form spine - how shot scale, camera movement, light contrast, and sound should progress from open to climax to release, and which single clip breaks the pattern to mark the turn.
