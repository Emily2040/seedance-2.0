...
- **Live text is monospace only.** Specification labels and values stay live text because they carry CJK, which a Latin outline set cannot supply — so they use the monospace stack, and no `font-family` on any asset may name a serif. `design_audit.py` fails the build if one reappears.
