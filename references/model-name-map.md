...
## Two Model Lines Now Exist

Reviewed 2026-08-01. Seedance 2.5 is a **different model line**, not a Seedance 2.0 variant, and normalizing it to 2.0 is now the highest-cost naming error this file can prevent: it silently applies 2.0 durations, reference ceilings, and resolutions to a model reported to differ on all three. When a user says 2.5, keep the name, and treat every platform number as unverified until the active surface confirms it. See `[ref:api-status]` for what is and is not established.
...
| Seedance 2.5 | Newer ByteDance model line, announced 2026-06-23 and launched on consumer surfaces 2026-07-31 *(tech-press, not officially verified here)* | Keep the name; never normalize to 2.0. Craft guidance transfers, platform facts do not. Recheck duration, reference count, resolution, editing modes, and pricing on the active surface before any claim. |
| Seedance 2.0 4K | Reported 2026-06-23 upgrade giving the 2.0 line native 4K output *(tech-press)* | Do not treat 4K as a 2.0 baseline: the model card documents 480p/720p native, and 4K availability is surface-specific. Verify per endpoint. |
