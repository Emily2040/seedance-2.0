...
This is the lightweight, canonical story-judgment contract. Load `[ref:directors-read]` before prompt compilation on every route, including interview, fast, short, sequence, and continuation routes. Do not recreate it from memory. The larger `[ref:directing-engine]` may deepen the camera, light, voice, and staging choices afterward, but it never replaces this gate.
